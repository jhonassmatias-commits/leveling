...
export default function App(){
  ...
  const timerStartRef=useRef(null); // timestamp when timer started
  const [timerSub,setTimerSub]=useState(null);
  const [elapsed,setElapsed]=useState(0); // updated every second from timerStartRef
  ...
  useEffect(()=>{
    (async()=>{
      ...
      // Restore timer if it was running before app was closed
      const savedStart=localStorage.getItem("rpg_timer_start");
      const savedSub=localStorage.getItem("rpg_timer_sub");
      if(savedStart&&savedSub){
        const startTs=parseInt(savedStart);
        const ne=Math.floor((Date.now()-startTs)/1000);
        if(ne<43200){ // max 12h
          timerStartRef.current=startTs;
          setTimerSub(savedSub);
          setElapsed(ne);
          setTimerOn(true);
          if(ne>=3600) m1.current=true;
          if(ne>=7200) m2.current=true;
        } else {
          localStorage.removeItem("rpg_timer_start");
          localStorage.removeItem("rpg_timer_sub");
        }
      }
  ...
  // Recalculate elapsed from start timestamp every second
  useEffect(()=>{
    if(!timerOn) return;
    const tick=()=>{
      if(!timerStartRef.current) return;
      const ne=Math.floor((Date.now()-timerStartRef.current)/1000);
      setElapsed(ne);
  ...
  // When app comes back to foreground, recalculate elapsed instantly
  useEffect(()=>{
    const onVisible=()=>{
      if(timerOn&&timerStartRef.current){
        const ne=Math.floor((Date.now()-timerStartRef.current)/1000);
        setElapsed(ne);
        if(ne>=3600&&!m1.current){m1.current=true;autoQuest("study_1h",30);}
        if(ne>=7200&&!m2.current){m2.current=true;autoQuest("study_2h",25);}
  ...
  const startTimer=(s)=>{
    if(timerOn) return;
    m1.current=false;m2.current=false;
    const now=Date.now();
    timerStartRef.current=now;
    localStorage.setItem("rpg_timer_start",String(now));
    localStorage.setItem("rpg_timer_sub",s);
    setTimerSub(s);setElapsed(0);setTimerOn(true);
  };
  const stopTimer=async()=>{
    if(!timerOn||!timerSub) return;
    setTimerOn(false);
    localStorage.removeItem("rpg_timer_start");
    localStorage.removeItem("rpg_timer_sub");
    const mins=Math.floor(elapsed/60);
    if(mins<1){setElapsed(0);setTimerSub(null);timerStartRef.current=null;return;}
    ...
    setElapsed(0);setTimerSub(null);m1.current=false;m2.current=false;timerStartRef.current=null;showToast(`Sessão salva: ${fmtHM(mins)}`,"#60a5fa");
  };
