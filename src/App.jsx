...
export default function App(){
  ...
  const timerOn=false; // kept for nav dot compat
  ...
  const logStudy=async()=>{
    if(!studySub||!studyStart||!studyEnd) return;
    const mins=calcMins(studyStart,studyEnd);
    if(mins<1){showToast("Horário inválido","#ef4444");return;}
    const c=charRef.current;
    const concs=c.concursos.map(cc=>{if(cc.id!==c.activeConcurso)return cc;
      const sm={...(cc.subjectMin||{}),[studySub]:((cc.subjectMin||{})[studySub]||0)+mins};
      return {...cc,subjectMin:sm,studySessions:[{date:todayStr(),subject:studySub,minutes:mins},...(cc.studySessions||[])].slice(0,40)};});
    const newStats={...c.stats,totalStudyHours:(c.stats.totalStudyHours||0)+(mins/60)};
    const newBoss={...c.boss,studyMin:(c.boss.studyMin||0)+mins};
    // XP: +30 per hour studied
    const xpGain=Math.floor((mins/60)*30);
    const newXP=Math.max(0,c.totalXP+xpGain);
    // Auto-complete study quests
    const newQ={...questsRef.current};
    if(mins>=60) newQ["study_1h"]=true;
    if(mins>=120) newQ["study_2h"]=true;
    setQuests(newQ);questsRef.current=newQ;
    const nc={...c,totalXP:newXP,stats:newStats,concursos:concs,boss:newBoss};
    charRef.current=nc;setChar(nc);addFloat(xpGain);triggerLvl(c.totalXP,newXP);
    await save(nc,newQ,null);
    setStudyStart("");setStudyEnd("");setStudySub(null);
    showToast(`${fmtHM(mins)} salvo! +${xpGain} XP`,"#60a5fa");
  };
  ...
  const css=`
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    html,body,#root{width:100%;min-height:100dvh;background:#07070f}
  ...
  return(
    <div style={{background:"#07070f",minHeight:"100dvh",fontFamily:"Crimson Text,Georgia,serif",color:"#e8dfc0",display:"flex",flexDirection:"column",width:"100%"}}>
      <style>{css}</style>
      ...
      <div style={{flex:1,overflowY:"auto",paddingBottom:68,position:"relative",zIndex:10}}>
        ...
        {tab==="study"&&<div style={{padding:"12px 12px 0"}}>
          ...
          {addingConcurso&&<div style={{display:"flex",gap:5,marginBottom:10}}><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={newConcName} onChange={e=>setNewConcName(e.target.value)} placeholder="Nome do concurso..."/><button className="tbtn" onClick={addConcurso} style={{background:"#1a1535",border:"1px solid #a78bfa55",color:"#a78bfa",borderRadius:8,padding:"9px 13px",fontFamily:"Cinzel,serif",fontSize:11,flexShrink:0}}>OK</button></div>}
          <STabs tabs={[{id:"timer",i:"📝",l:"Registrar",c:"#60a5fa"},{id:"questoes",i:"🎯",l:"Questões",c:"#a78bfa"},{id:"livros",i:"📖",l:"Livros",c:"#f0c040"}]} val={studyTab} onChange={setStudyTab}/>

          {studyTab==="timer"&&<>
              <Lbl>REGISTRAR SESSÃO DE ESTUDO</Lbl>
              {/* Subject selector */}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:12}}>
                {(ac?.subjects||[]).map(sub=><button key={sub.id} className="tbtn" onClick={()=>setStudySub(sub.id)} style={{width:"100%"}}><div style={{background:studySub===sub.id?`linear-gradient(135deg,${sub.color}30,${sub.color}10)`:`linear-gradient(135deg,${sub.color}14,${sub.color}06)`,border:`2px solid ${studySub===sub.id?sub.color:sub.color+"44"}`,borderRadius:11,padding:"10px",textAlign:"center"}}><div style={{fontSize:20,marginBottom:3}}>{sub.icon}</div><div style={{fontFamily:"Cinzel,serif",fontSize:8,color:sub.color,letterSpacing:1,marginBottom:2}}>{sub.name?.split(" ")[0]?.toUpperCase()}</div>{studySub===sub.id&&<div style={{fontSize:9,color:sub.color,fontFamily:"Cinzel,serif"}}>✓ Selecionado</div>}</div></button>)}
              </div>
              {/* Start / End time */}
              <Card style={{marginBottom:10}}>
                <Lbl>HORÁRIO DA SESSÃO</Lbl>
                <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:10}}>
                  <div style={{flex:1,textAlign:"center"}}>
                    <div style={{fontSize:9,color:"#555",fontFamily:"Cinzel,serif",letterSpacing:2,marginBottom:5}}>INÍCIO</div>
                    <input type="time" className="inp" autoComplete="off" value={studyStart} onChange={e=>setStudyStart(e.target.value)} style={{textAlign:"center",fontSize:20,fontFamily:"Cinzel,serif",color:"#60a5fa",padding:"10px 6px"}}/>
                  </div>
                  <div style={{fontSize:22,color:"#333"}}>→</div>
                  <div style={{flex:1,textAlign:"center"}}>
                    <div style={{fontSize:9,color:"#555",fontFamily:"Cinzel,serif",letterSpacing:2,marginBottom:5}}>FIM</div>
                    <input type="time" className="inp" autoComplete="off" value={studyEnd} onChange={e=>setStudyEnd(e.target.value)} style={{textAlign:"center",fontSize:20,fontFamily:"Cinzel,serif",color:"#f0c040",padding:"10px 6px"}}/>
                  </div>
                </div>
                {studyStart&&studyEnd&&(()=>{const m=calcMins(studyStart,studyEnd);const xp=Math.floor((m/60)*30);return m>0?<div style={{background:"#141228",border:"1px solid #2a2848",borderRadius:8,padding:"8px 12px",marginBottom:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontFamily:"Cinzel,serif",fontSize:13,color:"#e8dfc0"}}>{fmtHM(m)} estudados</span><span style={{fontFamily:"Cinzel,serif",fontSize:13,color:"#f0c040",fontWeight:700}}>+{xp} XP</span></div>:null;})()}
                <button className="sbtn" onClick={logStudy} style={{background:"linear-gradient(135deg,#1a1535,#221c42)",border:"1px solid #60a5fa55",color:"#60a5fa",fontSize:11,letterSpacing:2}}>✓ SALVAR SESSÃO</button>
              </Card>
          ...
          {studyTab==="questoes"&&<>
            ...
            <Card style={{marginBottom:10}}><Lbl>REGISTRAR QUESTÕES</Lbl><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5,marginBottom:6}}><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={qCorr} onChange={e=>setQCorr(e.target.value)} type="number" min="0" placeholder="Acertos"/><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={qTot} onChange={e=>setQTot(e.target.value)} type="number" min="1" placeholder="Total"/></div><select className="inp" value={qSub} onChange={e=>setQSub(e.target.value)} style={{marginBottom:8}}>{(ac?.subjects||[]).map(s=><option key={s.id} value={s.id}>{s.icon} {s.name}</option>)}</select><button className="sbtn" onClick={submitQ} style={{background:"linear-gradient(135deg,#1a1535,#221c42)",border:"1px solid #a78bfa55",color:"#a78bfa",fontSize:11,letterSpacing:1}}>✓ REGISTRAR (+0.5 XP/ACERTO)</button></Card>
        ...
        {tab==="body"&&<div style={{padding:"12px 12px 0"}}>
          ...
          {bodyTab==="treino"&&<>
            ...
            {workoutView==="edit"&&editPlan&&<div>
              ...
              <Card style={{marginBottom:10}}>
                ...
                <input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={editPlan.label} onChange={e=>setEditPlan(p=>({...p,label:e.target.value}))} placeholder="Nome da ficha" style={{marginBottom:6}}/>
                <input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={editPlan.focus} onChange={e=>setEditPlan(p=>({...p,focus:e.target.value}))} placeholder="Foco (ex: Peito · Tríceps)"/>
              </Card>
              ...
              {editPlan.exercises.map((ex,idx)=>(
                <Card key={ex.id} style={{marginBottom:7}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                    <input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={ex.name} onChange={e=>updateExField(idx,"name",e.target.value)} style={{flex:1,marginRight:8,padding:"6px 9px",fontSize:13}}/>
                    <button className="tbtn" onClick={()=>removeExercise(idx)} style={{color:"#ef4444",fontSize:16,flexShrink:0}}>🗑</button>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,marginBottom:5}}>
                    <div><div style={{fontSize:8,color:"#555",marginBottom:2}}>SÉRIES</div><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={ex.sets} onChange={e=>updateExField(idx,"sets",e.target.value)} style={{padding:"6px 8px",fontSize:12}}/></div>
                    <div><div style={{fontSize:8,color:"#555",marginBottom:2}}>REPS</div><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={ex.reps} onChange={e=>updateExField(idx,"reps",e.target.value)} style={{padding:"6px 8px",fontSize:12}}/></div>
                    <div><div style={{fontSize:8,color:"#555",marginBottom:2}}>DESCANSO</div><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={ex.rest} onChange={e=>updateExField(idx,"rest",e.target.value)} style={{padding:"6px 8px",fontSize:12}}/></div>
                  </div>
                  <input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={ex.tip} onChange={e=>updateExField(idx,"tip",e.target.value)} placeholder="Dica (opcional)" style={{fontSize:11,padding:"6px 9px"}}/>
                </Card>
              ))}
              ...
              <Card style={{marginTop:4,marginBottom:10}} glow="#34d39933">
                ...
                <input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={newExName} onChange={e=>setNewExName(e.target.value)} placeholder="Nome do exercício" style={{marginBottom:6}}/>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,marginBottom:6}}>
                  <div><div style={{fontSize:8,color:"#555",marginBottom:2}}>SÉRIES</div><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={newExSets} onChange={e=>setNewExSets(e.target.value)} style={{padding:"6px 8px",fontSize:12}}/></div>
                  <div><div style={{fontSize:8,color:"#555",marginBottom:2}}>REPS</div><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={newExReps} onChange={e=>setNewExReps(e.target.value)} style={{padding:"6px 8px",fontSize:12}}/></div>
                  <div><div style={{fontSize:8,color:"#555",marginBottom:2}}>DESCANSO</div><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={newExRest} onChange={e=>setNewExRest(e.target.value)} style={{padding:"6px 8px",fontSize:12}}/></div>
                </div>
                <input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={newExTip} onChange={e=>setNewExTip(e.target.value)} placeholder="Dica (opcional)" style={{marginBottom:9,fontSize:11}}/>
            ...
            {workoutView==="new"&&<div>
              ...
              <Card style={{marginBottom:10}}>
                <input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={newPlanName} onChange={e=>setNewPlanName(e.target.value)} placeholder="Nome da ficha (ex: Treino de Perna)" style={{marginBottom:7}}/>
                <input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={newPlanFocus} onChange={e=>setNewPlanFocus(e.target.value)} placeholder="Foco (ex: Quadríceps · Glúteo)" style={{marginBottom:10}}/>
                <Lbl>ÍCONE</Lbl>
          ...
          {bodyTab==="medidas"&&<>
            <Card style={{marginBottom:10}}><Lbl>ALTURA E PESO</Lbl><div style={{display:"flex",gap:5,marginBottom:8}}><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={weightIn} onChange={e=>setWeightIn(e.target.value)} type="number" step="0.1" placeholder={`Peso kg${char.body?.weight?` (${char.body.weight})`:""}`} style={{flex:1}}/><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={heightIn} onChange={e=>setHeightIn(e.target.value)} type="number" placeholder={`Altura cm${char.body?.height?` (${char.body.height})`:""}`} style={{flex:1}}/></div><button className="sbtn" onClick={saveBodyStats} style={{background:"linear-gradient(135deg,#0a2010,#0d2a14)",border:"1px solid #34d39955",color:"#34d399",fontSize:11,letterSpacing:1}}>💾 SALVAR MEDIDAS</button></Card>
        ...
        {tab==="life"&&<div style={{padding:"12px 12px 0"}}>
          ...
          {lifeTab==="financeiro"&&<>
            ...
            {addingExp&&<Card style={{marginBottom:10}}><Lbl>NOVO GASTO</Lbl><div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:7}}>{["🍽️","📱","🌐","💳","🏠","🎮","💊","📚","✈️","🎵","⛽","💄"].map(ic=><button key={ic} className="tbtn" onClick={()=>setNewExpIcon(ic)} style={{fontSize:17,opacity:newExpIcon===ic?1:0.3}}>{ic}</button>)}</div><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={newExpName} onChange={e=>setNewExpName(e.target.value)} placeholder="Nome do gasto" style={{marginBottom:5}}/><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={newExpAmt} onChange={e=>setNewExpAmt(e.target.value)} type="number" placeholder="Valor mensal (R$)" style={{marginBottom:5}}/><div style={{display:"flex",gap:5,marginBottom:9}}><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={newExpInst} onChange={e=>setNewExpInst(e.target.value)} type="number" placeholder="Parcela atual (ex: 2)" style={{flex:1}}/><input className="inp" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" value={newExpTotal} onChange={e=>setNewExpTotal(e.target.value)} type="number" placeholder="Total parcelas (ex: 12)" style={{flex:1}}/></div><div style={{display:"flex",gap:5}}><button className="tbtn" onClick={()=>setAddingExp(false)} style={{flex:1,padding:"9px",borderRadius:8,border:"1px solid #2a2848",color:"#666",fontFamily:"Cinzel,serif",fontSize:9}}>CANCELAR</button><button className="tbtn" onClick={addExpense} style={{flex:2,padding:"9px",borderRadius:8,background:"linear-gradient(135deg,#0a2010,#0d2a14)",border:"1px solid #22c55e55",color:"#22c55e",fontFamily:"Cinzel,serif",fontSize:10,letterSpacing:1}}>✓ ADICIONAR</button></div></Card>}
      ...
      {/* NAV */}
      <div style={{position:"fixed",bottom:0,left:0,width:"100%",background:"#09080f",borderTop:"1px solid #181530",display:"flex",justifyContent:"space-around",padding:"6px 0 10px",zIndex:100}}>
